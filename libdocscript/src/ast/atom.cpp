...
Atom::Atom(AtomType t, const std::string& c)
    : ASTNode(ASTNodeType::Atom), _type(t), _content(c)
{}

Atom::Atom(Token t) : ASTNode(ASTNodeType::Atom)
{
    _content = t.content;
...
Atom::Atom(const Atom& atom)
    : ASTNode(ASTNodeType::Atom), _content(atom._content), _type(atom._type)
{}
...
const std::string& Atom::content() const
{
    return _content;
}

Expression Atom::ptr_clone() const
{
...
ASTNode* Atom::rawptr_clone() const
{
...
AtomType Atom::convert_type(TokenType type)
{
    switch (type) {
...
AtomType Atom::special_type(const std::string& content)
{
