...
AtomType AtomHelper::get_type_by_token_type(TokenType tt) const
{
    switch (tt) {
...
AtomType AtomHelper::get_type_by_content(const std::string& content) const
{
    auto iter =
        std::find_if(_symbol_map.begin(), _symbol_map.end(),
...
Atom AtomHelper::create_by_type(AtomType t, std::string content) const
{
    auto iter =
        std::find_if(_symbol_map.begin(), _symbol_map.end(),
...
Atom AtomHelper::create_emptyline_symbol() const
{
