...
MacroExpander::MacroExpander(Macro& macro,
                             args_map& args,
                             Environment& env,
                             bool rec_expand)
  : _env(env)
  , _macro(macro)
  , _expand_recusrive(rec_expand)
  , _args(args)
{}
...
ast::Expression
MacroExpander::expand_1(Macro& macro, const args_list& args, Environment& env)
{
...
ast::Expression
MacroExpander::expand(Macro& macro, const args_list& args, Environment& env)
{
    auto map = create_map(macro.parameters(), args);
...
bool
MacroExpander::find_name(const std::string &name)
{
...
ast::Expression
MacroExpander::expand_expression(const ast::Expression& expr)
{
...
ast::Expression
MacroExpander::expand_atom(const ast::Atom& atom)
{
    if (atom.atom_type() != ast::AtomType::Symbol ||
        !find_name(atom.content())) {
        return atom;
    } else {
...
ast::Expression
MacroExpander::expand_list(const ast::List& list)
{
