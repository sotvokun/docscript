...
Interpreter::Interpreter(runtime::Environment& env)
  : _env(env)
{}
...
runtime::Value
Interpreter::eval(const ast::Expression& expr)
{
...
runtime::Value
Interpreter::eval_atom(const ast::Atom& atom)
{
...
runtime::Nil
Interpreter::eval_nil(const ast::Atom& atom)
{
    return runtime::Nil();
}

runtime::Value
Interpreter::eval_symbol(const ast::Atom& atom)
{
...
runtime::Boolean
Interpreter::eval_boolean(const ast::Atom& atom)
{
    if (atom.content() == "t") {
...
runtime::String
Interpreter::eval_string(const ast::Atom& atom)
{
    return runtime::String(atom.content());
}

runtime::Number
Interpreter::eval_number(const ast::Atom& atom)
{
    auto content = atom.content();
    ...
    if (content == "-nan") {
        return runtime::Number(
          -1 * std::numeric_limits<runtime::Number::dec_type>::signaling_NaN());
    }
...
runtime::Value
Interpreter::eval_list(const ast::List& list)
{
    if (list.is_textlist) {
        ...
        if (first.type() == ast::ASTNodeType::Atom) {
            ...
            if (type == ast::AtomType::Boolean || type == ast::AtomType::Nil ||
                type == ast::AtomType::Number ||
                type == ast::AtomType::String) {
                throw NotProcedure();
            }
...
runtime::List
Interpreter::eval_textlist(const ast::List& list)
{
    runtime::List result;
...
runtime::Value
Interpreter::eval_procedure_calling(const ast::List& list)
{
...
runtime::Value
Interpreter::eval_special_form(const ast::List& list)
{
...
ast::Expression
Interpreter::expand_macro(const ast::List& list)
{
